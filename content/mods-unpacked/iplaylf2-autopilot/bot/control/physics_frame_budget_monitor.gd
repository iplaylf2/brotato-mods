...
func build_context(
	scheduled_planner_count: int,
	planning_window_seconds: float,
	planning_loop_capacity_share: float
) -> Dictionary:
	...
	return {
		...
		"planning_loop_capacity_share": clamp(planning_loop_capacity_share, 0.0, 1.0),
