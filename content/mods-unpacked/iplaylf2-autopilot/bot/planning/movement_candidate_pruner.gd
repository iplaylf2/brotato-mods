...
func prune(admissible_actions: Array) -> Dictionary:
	...
	for candidate in admissible_actions:
		...
		if candidate.action.is_baseline_candidate:
			evaluation_actions.push_back(candidate)
			best_baseline_collision_risk = min(
				best_baseline_collision_risk, float(candidate.outcome.collision_risk)
			)

	# Preserve every equally best fine-lattice escape only when it improves on
	# the coarser baseline set. In an open field this adds no redundant work.
	if minimum_collision_risk < best_baseline_collision_risk:
		for candidate in admissible_actions:
			if (
				not candidate.action.is_baseline_candidate
