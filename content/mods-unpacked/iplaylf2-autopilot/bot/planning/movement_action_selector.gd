...
# Exclude a candidate only when its next committed interval contains a certain
# terminal collision and another candidate is not certain to terminate. Probabilistic
# contact and later consequences remain in the shared utility ledger, where attack,
# pressure relief, recovery, and exposure retain the same scoring semantics.
...
func select(scored_actions: Array) -> Dictionary:
	...
	var viable_actions := retain_committed_viable(scored_actions)
...
func retain_committed_viable(scored_actions: Array) -> Array:
	assert(not scored_actions.empty())
