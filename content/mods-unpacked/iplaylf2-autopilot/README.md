...
战略导航意图，每次只提交下一个短周期的移动输入。

Autopilot 根据当前物理帧余量和实测规划耗时分配搜索强度，并同步调整动作方向数、时间采样数、
导航基础方向数和武器预测数。帧样本尚不可用时保持冷启动基准；测得没有余量时降到仍能比较移动与攻击
机会的最低搜索形状，余量持续存在时再逐步提高搜索精度。
...
计划包含所选动作、移动方向、结果字段、字段级与目标级效用账本、动态上下文、导航意图、搜索预算和
近优动作摘要，供游戏内诊断与模型校准。
