...
Autopilot 是一个实验性 Brotato mod：它依据玩家能够获得的信息进行短视距滚动规划，并自动控制战斗
...
- 已实现可见世界观察、局内观察记忆、动态效用、轨迹搜索和移动执行闭环。
- 代码已经通过仓库格式、lint 和资源路径检查，但尚未完成游戏内加载与行为验证。
...
完整且不可放宽的规则见 [玩家权限边界](docs/fair-play.md)。
...
观察契约、结果字段、搜索预算、模块责任和当前限制见 [架构文档](docs/architecture.md)。
