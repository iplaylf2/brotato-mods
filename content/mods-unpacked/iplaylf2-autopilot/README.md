...
停留与各个移动方向，综合环境暴露、碰撞风险、资源、自动攻击机会、移动机制和战略导航意图；每轮只提交
一个短控制周期的移动输入，再用新观察重新规划。

动作方向与时间采样由玩家尺寸、移动速度、控制周期和可见弹道推导，不会因性能压力而降级。Autopilot
根据实测物理帧余量调整战略导航的基础方向数和进入完整武器预测的候选数：帧样本尚不可用时使用中等
细节，余量不足时逐级缩减可选计算，余量稳定恢复后再逐级增加。
...
计划包含所选动作、移动方向、结果字段、字段级与目标级效用账本、动态上下文、导航意图、计算细节预算和
近优动作摘要，供游戏内诊断与模型校准。
