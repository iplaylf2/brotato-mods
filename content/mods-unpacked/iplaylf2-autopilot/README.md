...
一个短控制周期的移动输入，再用新观察重新规划。规划在单一后台线程中运行，新结果就绪前沿用原移动输入；
...
生命被建模为即时生存缓冲与清场前补给库存。下一控制周期内，玩家完整动作集合与威胁运动的联合可达域
决定最强一击储备；动作预测窗和导航终点的环境暴露按扣除该储备后的即时缓冲计价。清场前可兑现的补给
只降低补给库存的边际价格，不会扩大眼前可承受的伤害。冒险是否值得仍由可兑现收益与候选动作风险在同一
账本中比较，不由波末时间窗或生命阈值触发。
...
`0.1` 秒控制周期计算，常规采样间隔约为 `1` 秒，实际间隔可能随物理帧调度和规划周转延长。样本写入
