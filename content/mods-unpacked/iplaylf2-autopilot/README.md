...
基础动作方向与碰撞时间采样由玩家尺寸、移动速度、控制周期和相关弹道推导，不会因性能压力而降低。
Autopilot 根据实测物理帧余量为本轮规划设置计算截止。基线导航、碰撞投影和至少两个候选动作的完整
武器预测不会因截止而省略；近处目标方向、移动角区间细分和更多候选的武器预测则在对应阶段仍有余量时
继续扩展。无法在规划时域内影响玩家可达区域的投射物不会进入重复战术计算，但仍保留在原始观察和采样中。
...
计划包含所选动作、移动方向、结果字段、字段级与目标级效用账本、动态上下文、导航意图、计算预算、
投射物过滤诊断和近优动作摘要，供游戏内诊断与模型校准。
