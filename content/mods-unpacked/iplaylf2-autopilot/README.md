...
物理帧余量和近期规划耗时设置计算截止。近场动作在所有质量档都保留八方向安全基线；持续接近预算时，
系统优先减少远场导航方向和预算内细分，导航基线由八方向逐级降至六方向、四方向。只要有足够的可行候选，
前两个候选仍会接受完整武器预测，使攻击结果能够改变动作选择；其余机会方向、角区间细分和武器预测只在
对应阶段仍有余量时继续。恢复余量后，搜索质量自动提高。无法在规划时域内影响玩家可达区域的投射物不会
进入重复战术计算，但仍保留在原始观察和采样中。
