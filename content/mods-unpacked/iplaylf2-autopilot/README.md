...
继续生效。控制器只提交不含场景节点的隔离值快照和帧预算上下文并应用完成结果；`PlanningWorker` 在
后台线程创建并独占规划器状态，同时管理等待、数据交接和关闭。工作线程无法启动时，Autopilot 不接管
移动；运行中拒绝规划请求时，Autopilot 恢复原移动行为。两种情况都会记录错误，且不会转到物理线程
执行规划。
...
随之恢复。计算预算和候选分配的具体契约见 [架构文档](docs/architecture.md#计算预算与搜索保真度)。
...
生命被建模为清场前可补充的库存：可兑现的生命补充降低后续环境暴露和储备消耗的机会成本，但动作预测窗
内的碰撞先按即时生存缓冲计价，未来补充不会扩大眼前可承受的伤害。冒险是否值得由尚可兑现的材料、击杀
和恢复收益与候选动作的实际风险共同决定，不由波末时间窗或生命阈值触发。
