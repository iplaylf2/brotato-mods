...
计算成本外推的预计生效状态开始比较。任一线程不可用时不接管或立即交还移动控制。线程所有权、数据隔离和失败处理见
...
不依赖固定的“一击或两击致死”判断。计算压力可以收缩近似战略导航的均匀扫描和额外搜索覆盖，
但不改变由碰撞几何派生的局部动作基线、时间采样或保留候选的结果语义。选择器只排除下一次重规划前确定且
可避免的死亡；其余承伤风险会随当前持有材料、道具和武器的替代价值连续计价，并与交战、恢复和资源收益共同决定方向。
