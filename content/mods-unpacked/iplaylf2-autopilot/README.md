...
- 当前源码已通过可移植静态检查、Godot 3.7.dev 脚本编译检查、模型语义契约检查和持久规划工作线程的
  生命周期冒烟测试。
...
继续生效。控制器只提交不含场景节点的隔离值快照并应用完成结果；线程的等待、数据交接和关闭由独立
工作模块管理。工作线程无法启动或拒绝请求时，Autopilot 会记录错误并恢复原移动行为，不会转到物理
线程执行规划。
