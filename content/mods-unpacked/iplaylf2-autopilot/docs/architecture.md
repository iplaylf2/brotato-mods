...
规划不消费的机制证据副本。控制器把快照和对应规划器提交给 `PlanningWorker`；工作线程只执行规划并返回值结果，
控制器在主线程整体替换计划。`PlanningWorker` 独占 Godot 3 的长期线程、信号量、互斥交接和退出协议，
不读取或回调活动场景树。工作线程无法启动或拒绝请求时，控制器记录错误并恢复原移动行为，不在物理
线程降级执行规划。关闭时，控制器先回收工作线程，再释放观察与采样边界。
