...
5. 每个动作只在上述敌人交互域中生成一次战场、速度障碍和事件基础结果，再从共享的
   ...
   间隙。解析速度空间证据只保留弹道 TTC、队友碰撞和已知冲撞的锁定走廊，最终风险取两类证据的较大值，
...
6. 动作预测窗内的峰值碰撞与累计接触证据先按可见威胁伤害、候选移动状态下的护甲/闪避、命中保护、原版
   最短无敌帧间隔和当前生命换算为预期生命损失；非致命生命成本对消耗后的预计生命库存积分，越接近库存
   耗尽，边际成本越高。超过当前即时生存缓冲的部分另以终止风险价格进入同一效用账本。
...
| `field_utility_breakdown.forecast_health_inventory_loss_value` | 效用模型对动作预测窗内的非致命生命消耗沿预计生命库存的对数障碍积分，再对超过即时生存缓冲的部分追加终止成本后形成的负向贡献 |
| `forecast_terminal_collision_risk` | 单次命中或动作预测窗内的累计接触可能耗尽当前生命时保留的条件终止风险；按 `terminal_health_loss_unit_value` 进入生存效用 |
...
| `forecast_expected_health_loss` | 动作预测窗内的峰值碰撞风险与累计接触按原版最短无敌帧间隔换算预期命中次数，再结合最大单次伤害、候选护甲、闪避和命中保护得到的条件生命消耗；效用模型将它换算为 `forecast_health_inventory_loss_value` |
...
树木与敌人不以外观半径扩大射程。锁定资格本身会在进入射程后变成平坦值，因此导航把中心距离保留为
玩家塑造后续目标可达性的通用代理；精确的最近目标分配仍只归局部武器结果场。导航由此保留接近树木或
高移除价值敌人的连续梯度，具体吸引强度来自统一奖励、战场后果和攻击容量账本，而非树木或敌人类别
优先级。敌人机会用起点、中点和终点的可达性代理近似沿途积分，使候选提前形成后续攻击机会获得价值。
...
降低；但未来补充不会扩大即时生存缓冲，也不能吸收它到达前的致死伤害。可能结束本局的碰撞按
`terminal_health_loss_unit_value` 计价，并同时考虑最强单次命中和动作预测窗内按原版最短无敌帧间隔折算
的累计接触。因此波末、恢复构筑或地图补充充足时的冒险，都来自可兑现收益与实际风险的统一比较，而不由
专用时间窗或生命阈值触发。

拾取消耗品把地图生命补充转化为当前生命。`expected_recovery` 记录动作预测窗内的条件恢复收益，
`consumed_consumable_recovery_supply` 结清离开地图的储备价值；两者在同一账本中形成从储备到当前生命的
净变化。可兑现恢复量受缺失生命上限约束。
...
概率按波末道具选择的最低回收价值计价；所有可能生成的消耗品都按当前生命补充的转化边际价值计价，
...
`StatOpportunityValueModel` 按当前属性、机会曲线和剩余波次计算边际价值。账本构造只扫描敌群一次，
...
短期执行诊断而不另行计分。弹道与队友 VO 使用连续 TTC 风险，已知冲撞使用锁定走廊；终止与非终止风险
都在同一效用账本内和其他结果交换，不另设硬筛选。
...
拥有“当前生命、即时威胁与清场前补充 → 生命库存及单位价值”的协议，供导航风险和动作效用共同消费。
...
- `bot/planning/battlefield_influence_model.gd` 拥有环境暴露、位置域碰撞风险、战斗支援伤害与消耗、友方减压、
  ...
  协作者消费。
- `bot/planning/velocity_obstacle_risk_model.gd` 计算弹道与队友 TTC 及已知冲撞扫掠走廊的局部速度空间交会
  风险，最终碰撞风险由动作结果预测器与普通敌人、投射物的位置域扫掠证据合并；
