...
3. 导航风险使用与局部生存效用相同的环境风险单位价值。当前生命、防御、最强可见下一击储备和清场前
   可兑现的生命补充共同决定该价值；本波剩余时间只影响补充与收益能否兑现，不直接折价生命。
...
暴露、远期碰撞诊断或导航机会势能表达。`material_acquisition_value` 只拥有候选路径上的实际收集；尚未
拾取材料的空间价值由导航统一聚合。导航以原版收集半径作为材料交互边界，并按直线路径到各实体的最近点
计值，因此一条路线扫过的材料会全部贡献价值，进入吸附范围但尚未收集的材料仍会保留机会梯度。
...
候选与零输入基线在各时刻使用相同运动机制，
因此敌人自行靠近只改变共同未来状态，不会伪装成移动收益。导航环境暴露也比较同一终点时刻的反事实差值。
终点总增益只决定导航方向和总行程价值；动作评分再按本次输入相对零输入
...
消耗品按到达时间和存在置信度折算，概率掉落按目标完成可行性折算，被动恢复与生命偷取按剩余可作用时间
折算。`HealthInventoryValueModel` 将这些补充与当前生命、被动生命流失和最强可见下一击合成为库存价值；
本波剩余时间只界定补充与收益能否兑现，不直接改变风险偏好。具体公式和校准证据见
[生命库存定价](model-calibration.md#生命库存定价)。

当前生命扣除最强可见下一击的储备后形成 `immediate_survival_buffer`；该缓冲加上清场前可兑现补充、再
扣除同期被动生命流失，形成 `projected_health_inventory`。动作的非致命生命成本沿预计库存的对数障碍
积分，越接近库存耗尽，边际成本越高。补充充足时，承伤扩大收益的机会成本自然降低；但未来补充不会扩大
即时生存缓冲，也不能吸收它到达前的致死伤害。可能结束本局的碰撞按
...
比例兑现；导航中的敌人机会再乘剩余时间内的可击杀性。击杀或效果规则造成的属性变化由
...
与目标无关的期望攻击容量”协议，该容量同时由战斗、机会与生命补充模型消费。`WeaponOutcomeFieldModel`
...
- `bot/planning/velocity_obstacle_risk_model.gd` 计算弹道与队友 TTC 及已知冲撞扫掠走廊的局部速度空间交会
  ...
  单位价值；`bot/planning/target_completion_feasibility_model.gd` 统一提供敌人击杀和树木摧毁可行性，供
  机会定价与恢复掉落预测共同消费；
...
- `bot/observation/observed_motion_estimator.gd` 负责跨帧运动测量，
  ...
  `bot/planning/motion/enemy_motion_predictor.gd` 对稳定目标位置响应逐步积分，并在不适用时回退到观测运动
  外推。普通敌人接触扫掠归战场影响模型所有；冲撞时序仍由观察层拥有，候选路径与锁定走廊的交会则由
  速度障碍模型拥有；观察层不得反向依赖规划层。
