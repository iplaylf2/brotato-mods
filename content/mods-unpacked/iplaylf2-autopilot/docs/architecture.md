...
只给出稳定区间。冲撞风险按锁定位置与高速扫掠走廊的候选路径交会计算，因此横向脱离、沿线停留或径直
后退的差异来自几何预测，不编码闪避动作方向。这些版本知识无需以本局受击为前提。
...
2. 导航意图规划器以同一终点总价值比较原点、均匀基线方向、正价值机会的精确方向和额外角区间；最强
   机会方向属于有界基线，其余机会方向和角区间受截止约束。每个终点同时计算材料、恢复、树木、敌人、
   地图信息和环境暴露的价值差；
   动态敌人与环境暴露都以同一未来时刻的零输入状态为反事实基线，只把玩家移动实际改变的部分归给动作。
   地图项使用候选视口内各观察网格的面积和观察陈旧度：未见网格具有完整再观察价值，已见网格随距上次
   观察的时间连续恢复该价值。因此未知区域产生探索，地图全部定位后也会自然巡游较久未观察的区域，无需
   预设巡逻路线。只有正增益终点会形成移动偏好。
...
4. 动作只会执行到下一次重规划。提交期由整数物理 tick 派生；近端预测窗为两个控制步。
   ...
   相位步长不超过 `π/2`。
5. 每个动作只生成一次战场、速度障碍和事件基础结果，再补全自动武器攻击后进入唯一一次完整评分。
   碰撞风险取位置域扫掠证据与速度空间 TTC 证据的较大值，同一次
   交会不会因两个检测器都发现它而相加；位置域投射物使用扫掠线段，避免高速弹体穿过采样间隙。
6. 峰值碰撞与累计接触证据先按可见威胁伤害、候选移动状态下的护甲/闪避、命中保护、原版最短无敌帧
   间隔和当前生命换算为预期生命损失；生命成本对消耗后的有效生存缓冲积分，越接近最强可见下一击所需
...
局部未实现进度和预测窗外的导航终端值。一个事实可以被多个预测器观察，但只能由一个
评分量拥有其决策含义：位置扫掠和 VO 都是碰撞检测证据，合并后才计分；实际拾取消耗品形成
`expected_recovery`，并由 `consumed_consumable_recovery_supply` 结清离开地图的恢复储备；未触发拾取的
靠近才形成 `consumable_recovery_approach_progress`，满血或溢出拾取造成的恢复机会损失另由
`wasted_consumable_recovery` 诊断。不可逆拾取只在本次实际提交的控制期内确认为
...
也不会抵消即将拾取的机会；同一状态势能还保证反复靠近后离开不能刷取收益。
...
直接压力和持续机制后果先合并为敌人移除价值，再由实际伤害和接近共同消费。树木接近只表达空间
...
| `terminal_collision_risk` | 单次命中或预测窗内的累计接触可能耗尽当前生命时保留的终止碰撞风险；按未折价生命价值进入生存效用 |
...
| `expected_health_loss` | 峰值碰撞风险与沿途累计接触按原版最短无敌帧间隔换算预期命中次数，再结合最大单次伤害、候选护甲、闪避和命中保护得到的预期生命消耗；效用模型再将它换算为 `health_resource_loss_value` |
...
时域和已知边界裁剪。地图信息场以观察网格覆盖面积计值：未见网格具有完整再观察价值，已见网格按距
上次观察的时间相对波长连续恢复该价值。它只表达合法视野的预期信息收益，不把未知区域视为已知安全区，
也不编码巡逻路线。
...
同时考虑最强单次命中和预测窗内按原版最短无敌帧间隔折算的累计接触。拾取消耗品把地图替代供给转化为
...
计分。VO 使用连续 TTC 风险；终止与非终止风险都在同一效用账本内和其他结果交换，不另设硬筛选。
...
武器预测读取玩家自身武器的 `timing`、`delivery`、`impact` 和 `rules`，让当前可见敌人与树木共同参与
原版最近合法目标选择，再计算攻击时刻、路径碰撞与命中事件。锁定目标只决定瞄准方向；路径上更早相交的
实体仍可成为首个命中目标。每个射击时刻先以保守机动上界排除不可能进入查询域的目标，再对保留目标执行
运动外推和路径几何验证。弹射、范围效果与派生投射物从实际命中事件继续传播，并分别遵守目标选择、事件
锚点、剩余飞行时间和最大飞行距离。随机选中不可达目标的概率质量不重新分配给近处目标，伤害与移除价值
也始终按事件对应的具体目标结算。

敌人命中按目标移除价值计分，树木命中按破坏所需命中数折算收获进度。同一预测窗的敌人伤害、击杀权重
和移除价值不能超过可见敌人的总耐久与总价值，树木收获进度也不能超过可见树木的总收获价值。原版的
装填、扩散、贯穿、弹射、近战形态和命中后效果只在机制编译边界变换成公共攻击模型；预测结果只参与移动
动作效用。
...
启动。搜索保真度决定保留多少动作候选；每个保留候选始终执行相同的武器、碰撞和事件预测，计算压力
不会切换结果语义。
...
规划消费者的统一最大机动能力契约，因此不会从规划观察中全局延后；武器预测只在具体投送域内使用当前
轨迹、已编译运动能力和不确定半径进行保守宽相过滤。
...
造成的位置响应反事实。自动武器还按候选动作与射击时刻复用不可变的目标查询快照：快照以保守距离界作
宽相过滤并只对保留目标执行精确运动外推，攻击预测器随后验证路径和事件几何。两类缓存均不跨物理帧，
不改变时域、采样数、候选数或攻击语义。
...
运动观察 → 未来位置”的协议，供暴露、交会、事件与动作采样共同消费；`weapons` 拥有“攻击模型 → 射击
时序与可达命中结果”的协议，其中射击速率同时由战斗、机会与生命资源模型消费，完整攻击结果由动作结果
预测器消费。其余协作者在出现第二个独立规划入口或同等级稳定协议之前继续留在规划根目录。
...
- `bot/knowledge/weapons/weapon_mechanic_compiler.gd` 拥有目标版本武器状态与资源到 `attack_model` 的映射；
  `bot/planning/weapons/weapon_fire_model.gd` 统一定义与目标无关的射击时序、单次命中伤害，以及命中、伤害和
  生命偷取速率，
  `bot/planning/weapons/weapon_target_query_model.gd` 拥有目标快照、宽相过滤和空间查询，
  `bot/planning/weapons/weapon_attack_predictor.gd` 负责已获准候选动作的目标选择与命中事件传播。
...
- `bot/planning/map_information_value_model.gd` 根据视口、已观察边界、覆盖陈旧度和候选终点计算新观察与
