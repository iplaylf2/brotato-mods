...
控制器同步截取只读规划快照；其中只含已经脱离场景节点的稳定嵌套值，不携带规划不消费的机制证据副本。
该不可变快照交给单一后台规划线程；物理线程在结果尚未完成时继续提交上一移动输入，结果就绪后再整体
替换计划。关闭控制器时会先等待在途规划结束，再释放观察与采样边界。
...
| `planning_turnaround_usec` | 从后台规划开始到主线程接收并准备应用结果的总周转时间，包含计算、线程调度和交接等待 |
