...
7. 在动作细分截止前，规划器围绕当前高分动作的相邻角区间插入中点，逐个执行碰撞投影和不含武器攻击的
   ...
   候选仅在最终规划截止前继续预测。未做精确武器预测时，进入射程的敌人移除价值和树木收获价值分别
   作为同一战斗或经济价值的筛选代理；精确结果出现后，对应代理退出评分。
...
造成的恢复机会损失形成 `wasted_consumable_recovery`。材料的实际收集与所有局部材料的价值加权接近分别由
`material_acquisition_value` 和 `material_approach_progress` 拥有；前者计入未在本波收集、推迟至后续波次
兑现造成的连续成长时机成本，后者是可正可负的空间势能差，不能通过反复靠近后离开刷取收益。
...
| `material_approach_progress` | 对所有尚未拾取的局部材料，按价值聚合的有符号收集势能变化 |
...
消耗品、剩余时间内的掉落机会和被动恢复，为当前生命及恢复储备分别计算边际价值。非致命生命和恢复
储备只服务于本波剩余暴露，其价格随剩余暴露时域连续下降；可能结束本局的碰撞仍使用未折价的生命价值，
并受终止风险可行域保护。恢复充足时价格下降，接近终止余量且供给稀缺时价格上升。拾取消耗品会消耗
未来恢复储备，实际恢复量受缺失生命上限约束。
...
所需命中数折算收获进度。原版的装填、扩散、贯穿、弹射、近战形态和命中后效果只在机制编译边界变换成
这些量。预测结果只参与移动动作效用。
