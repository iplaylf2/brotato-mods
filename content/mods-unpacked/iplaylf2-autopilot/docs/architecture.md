...
3. 导航图逐层执行 Bellman 最短路递推，得到累计 `path_cost`、父节点和 `route_value`，因而远场目标
   必须支付中间暴露带的代价。该代价使用与局部生存效用相同、随生命和防御变化的环境风险价格，导航层
   不定义独立的风险偏好。动作生成器离散化本次可提交的移动输入；零向量是不输入移动指令的动作。
...
和最长 1.2 秒导航预测期。导航图在局部预测可达半径内不计残余终端收益：当前可见且局部可达的静态目标
交给动作预测器，不可见的记忆目标由导航图的即时终端值负责，避免把动作预测器无法读取的目标丢在责任
交界处。治疗和交战只能从局部窗口结束后的持续价值开始计入。
...
网格污染局部碰撞判断。局部预测半径内的节点仍参与路径成本；当前可见的局部目标由动作预测器计分，
不可见的记忆目标则保留导航即时终端值。跨出局部半径后，其余终端收益才逐步生效。规划结果同时公开
推导出的 `near_node_spacing`、`local_detail_radius`、`local_prediction_radius`、`control_distance`、
`engagement_capacity` 和 `current_engagement_estimate`，便于验证空间尺度是否合理；每个节点的
`engagement_estimate` 记录该位置的预计攻击数、命中数和伤害，`unowned_local_strategic_value` 记录
动作预测器无法消费的局部记忆目标价值。

导航路径的环境暴露按局部生存效用中同一个动态风险价格归一化后计入通行成本。满生命且有生存余量时，
风险价格允许远处材料、战利品目标和接敌机会参与路线竞争；生命或防御不足时，局部与导航时域同步提高
风险价格。边界压力从默认局部预测期所对应的机动距离开始增长，为转向保留空间。
...
搜索强度的政策取值域为 `[1, +∞)`；每轮可持续目标值由规划耗时预算除以单位搜索成本得到。
...
定义反馈的收敛速度与稳定性。Godot 性能监视器可能短暂延迟，控制层因此跳过紧随规划之后的基线样本，
并以规划器自身的单调时钟测量闭合反馈。反馈在下一轮规划生效，因而首次规划、同一帧内的突发负载和
监视值更新延迟仍可能造成超预算。
...
- `bot/planning/battlefield_exposure_model.gd` 拥有环境暴露、位置域碰撞风险、友方减压、挡弹时序和动作诊断；
  `bot/planning/navigation_value_graph_builder.gd` 为局部预测未拥有的目标和动作窗外的残余价值提供导航，
  并求解 Bellman 累计价值。三段时间边界由 `bot/planning/movement_planning_timing.gd` 唯一定义，体型、
