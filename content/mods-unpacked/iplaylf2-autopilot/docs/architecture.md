...
5. 每个动作只在上述敌人交互域中生成一次战场、速度障碍和事件基础结果，再由
   [期望武器结果场](#期望武器结果场) 沿该动作路径计算下一决策状态的修正，然后进入唯一一次完整评分。
   候选方向不会展开逐发攻击过程。
...
导航路径势能表达。空间机会模型只为射程外的攻击窗口计价；进入射程后，原版最近目标规则及移动造成的
目标切换完全归期望武器结果场所有。该结果场按最近目标顺序分配实际命中容量，并按剩余敌人生命或树木
所需命中数折算完成进度，不包含树、精英、宝箱怪或 Boss 的身份优先级。
...
缺口会产生连续的战略价值；进入范围后，空间机会保持饱和，不再重复给最近目标切换计价。局部武器结果场
直接沿候选路径解释原版自动选靶，因此移动仍可让高价值目标自然获得火力，但不会从战略与局部边界领取
两份相同收益，也不会单因身份追逐其中心。吸引强度来自统一奖励、战场后果和共享攻击容量，而非树木
或敌人类别优先级。敌人机会用起点、中点和终点的可达性
...
反事实。期望武器结果场按候选动作直接评价路径，不跨帧保留地图或目标状态。
