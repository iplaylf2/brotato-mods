...
1. 控制器提供由实测物理回调峰值形成的帧预算上下文。规划器先用保守位移上界排除在导航时域内无法影响
   ...
   局部均匀动作、额外角区间细化和机会评价。零输入、导航选出的价值方向和碰撞时间采样始终保留；因此
   局部动作数量可以降级，而导航发现的机会或低暴露方向仍能进入完整评价。
...
6. 动作预测窗内的峰值碰撞、累计接触和解析交会先保留可叠加的接触证据总量与原始伤害加权证据总量；
   ...
   当轮保留的均匀基线与导航偏好方向继续完整计分，不存在跨语义模型的质量档。
...
方向。全局导航固定保留八个均匀格点，并始终保留最高价值机会方向。`PlanningSearchFidelityAllocator`
