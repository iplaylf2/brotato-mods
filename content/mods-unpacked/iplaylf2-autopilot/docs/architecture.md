...
2. 导航意图规划器在最长 1.2 秒可达圆内评价一组均匀方向，并补入已观察资源与战略敌人的精确方向。
...
`navigation` 和 `control_stability`。调试构建在评分上下文生成时断言同一字段不能进入两个目标；
...
和最长 1.2 秒导航预测期。当前可见且位于局部预测可达半径内的静态目标由动作预测器负责；导航意图
...
随机抽样位置和所选概率。这些诊断只用于解释校准样本和最终选择，不参与效用评分。
...
| `tree_engagement_progress` | 进入树木攻击范围或向其接近的进度 |
...
导航终点的环境暴露按局部生存效用中同一个动态风险价格计入成本。动作评价仍查询完整连续路径并执行
扫掠碰撞检测，所以战略终点不会替代战术避障。候选动作另外诊断 `[P(x(T),T)-P(x(0),0)]/T`，它是
沿动作的平均物质导数，对应 `∂P/∂t + v·∇P` 的路径平均。各通道的正负
...
权重在每次规划时重建。波次接近结束时，可见材料和战利品目标价值上升；健康角色还会按波次进度逐步
把部分当前生命作为只服务于本波机会的风险预算，降低环境暴露与碰撞的交换价格；会让正伤害直接致死
的规则仍将该预算归零。生产型敌人的提前处理价值随剩余时间增加。生命比例、护甲、闪避和恢复共同形成
风险容忍度；规则投影产生的事件价值、状态变化和长期保留价值通过同一账本调整对应结果，不切换到
内容专属策略。
...
导数和两个碰撞检测器的原始结果只保留为诊断账本。敌人轨迹的位置不确定半径只扩大邻近压力，不扩大
VO 的实体碰撞圆；物理碰撞几何始终使用玩家与敌人的实体半径，避免把“可能位于某处”误写成“TTC 为零”。
...
搜索强度的政策取值域为 `[0.25, +∞)`；每轮可持续目标值由规划耗时预算除以单位成本得到。没有帧耗时
样本时保持冷启动基准强度；已有样本且预算为零时才表示已测得没有余量，并降到最低强度。
各分辨率按搜索强度的平方根增长，使方向数与时间采样数的乘积近似随总强度线性增长：
...
导航基础方向 = round(12 × 分辨率倍率)
```

最低强度仍保留四个动作方向、两个时间采样、两个完整武器预测和六个导航方向；分辨率倍率可随持续
实测余量继续提高这些离散维度。
...
最低搜索形状保留候选比较所需的各维度；强度下界为 `0.25`，基准强度 `1` 定义正常搜索形状。响应系数、
EMA 时间常数和单次调节范围定义反馈的收敛速度与稳定性。Godot 性能监视器可能短暂延迟，控制层因此
跳过紧随规划之后的基线样本，并以规划器自身的单调时钟测量闭合反馈。反馈在下一轮规划生效，因而首次
...
`NavigationIntentPlanner`。`bot/planning` 当前保持单一目录，因为这些协作者共同服务唯一入口
`MovementPlanner`，且存在密集的包内依赖；在出现第二个独立规划入口或可单独消费的稳定子协议之前，
拆分子目录只会制造未经契约支持的结构边界。
...
- `bot/planning/battlefield_exposure_model.gd` 拥有环境暴露、位置域碰撞风险、友方减压、挡弹时序和动作诊断；
  ...
  三段时间边界由 `bot/planning/movement_planning_timing.gd` 唯一定义，体型、
  速度与这些时域形成的共享空间尺度由 `bot/planning/movement_scale_model.gd` 统一派生。
