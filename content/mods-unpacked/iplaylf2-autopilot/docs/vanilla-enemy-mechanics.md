...
敌人的基础材料、消耗品掉率、必掉约束和击杀造成的玩家属性变化同样在可见后编译为 `kill_rewards`；
`evil_mob` 的击杀结果包含诅咒 `+1`。规划层按当前幸运、目标最大生命、武器输出和剩余时间给击杀结果
定价。目标版本的诅咒概率曲线为 `limit × (1 - 1 / (1 + curse / 50))`：诅咒敌人概率上限为 `0.5`，
...
8. 战利品敌人的基础材料、消耗品掉率、必掉约束和诅咒收益是否变化；
