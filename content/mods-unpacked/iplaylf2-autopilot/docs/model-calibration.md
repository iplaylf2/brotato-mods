...
搜索计算分配，不代表碰撞采样精度、动作结果优劣或目标价值。导航基线除均匀方向外，至多再保留一个
不与格点重合的最高机会价值上界方向；该方向属于 `baseline_position_evaluation_count`，不属于截止准入
控制的额外评价。
...
| 经验启发式 | 生命库存的对数障碍尺度与生命补充影子价格、目标完成概率的竞争分配、材料推迟兑现的成长时机价值、材料吸收竞速时域与成长负担代理、地图再观察价值、诅咒净机会价值基准、预算压力指数、搜索保真度下限、额外工作额度、耗时 EMA 与截止保护倍率、运动估计平滑与遗忘阈值、目标响应积分的位置误差目标与最小步长 | 不宣称物理真实性；通过当前实现产生的样本校准 |
...
保留四个均匀方向。全局导航保留八个均匀方向和最高价值派生方向，后者也会进入局部候选。时间采样数同时
满足相邻玩家位移不超过 `2r`、每个控制期至少一个样本、
