...
缓存不进入采样。导航意图还保留至多一个由当前最高观察机会派生的局部动作方向；它只确保完整动作模型
评价该方向，不绕过最大效用选择。
...
自动武器预测按候选动作与射击时刻复用不可变的目标查询快照。快照先以敌人的当前距离、机动上界、半径
和事件最大传播距离做保守宽相过滤，再只对保留目标执行运动外推；路径和事件几何仍由攻击预测器逐次验证。
同一时刻的多把武器仍分别执行完整攻击语义、规则与上限结算，但普通直击不再扫描或外推无法进入本次
投送域的远处敌人；弹射和派生投射物只在剩余飞行时间允许时向快照中的局部目标传播。性能复盘应分别
统计武器数、不同射击时刻数、敌人数及武器的重定向/派生投送能力；武器数不再等价于目标运动外推次数。
敌人运动预测还按当前物理帧、轨迹和预测时刻复用无输入基线；候选位置响应仍逐动作计算，因此缓存与宽相
只消除无贡献工作，不降低搜索保真度。
...
`MovementPlanner.plan()` 开始，到动作选择完成为止。完整物理帧验证还要覆盖随后的采样序列化、文件
写入、控制器收尾与游戏帧表现。
...
| 等级 | 当前参数族 | 处理原则 |
| --- | --- | --- |
| 原版规则或运行时测量 | 碰撞半径、拾取半径、武器时序、锁定距离与最大飞行距离、候选状态下的可执行移动速度、护甲减伤公式、一级属性升级增量、诅咒概率曲线、原版每物理帧以 `0.1` 为权重衰减击退所对应的指数衰减率 | 保持原版或观察为唯一真相源；升级目标版本时复核 |
| 由状态与统一控制契约派生 | 控制/近端/局部/导航时域、有效时域、动作方向数上限、时间采样数、控制距离、敌人/投射物压力距离、遇敌余量、边缘与队友机动余量、漫游归一距离、机会可达距离、局部预测半径、材料份额交接区、TTC 风险与截断时域 | 不独立调参；只复核物理 tick 数、控制步数、碰撞直径倍数或解析弹道相位分辨率 |
...
2. `MovementUtilityModel` 使用材料等价总账：一单位材料的基础价值为 `1`；收集地面材料时，
   ...
   实际成长收益和各价值模型的兑现误差。尚未拾取材料的接近势能聚合可达前沿；容量由
   `opportunity_reach_distance / (2 × collection_radius)` 向上取整，不另设材料数量阈值。会吸收材料的
...
3. `PlanningComputeBudgetPolicy` 的单项耗时 EMA、截止保护倍率和双偏差基线余量，以及
   ...
   次数反推。
4. `OpportunityValueModel` 中的树木收获可行性、材料兑现价格和消耗品掉落价值，以及
   `HealthResourceValueModel` 中的恢复展望窗、基础生命价值、生存余量曲线和非致命生命折价，仍需共同
   校准。样本应覆盖树木与特殊敌人的实际兑现率、不同构筑下各类恢复供给的兑现率、末段受伤频率和生命
   收益交换。非致命生命按本波剩余比例折价；恢复展望窗只界定近期替代供给，不得再次充当波末折价窗口。
...
4. 检查最多三个高分候选的分数跨度、字段账本、最大效用所选动作和方向连续性。若方向频繁反转，应先
   检查重复计分、单位归一化、局部机会聚合、观察网格陈旧度和候选角向覆盖，不能在选择器外再加目标
...
5. 材料复盘按本波剩余比例分段，比较可见材料数、实际材料增长、`material_acquisition_value` 和有符号
   ...
   密集或跨越份额交接区时，重点比较逐控制期反转率。出现 `material_assimilation.active` 的敌人时，还应
   比较材料与玩家、敌人的预计到达次序、后续实际材料消失及候选排序；没有可争夺材料时，该画像本身不应
   形成固定击杀优先级。
6. 树木复盘同时比较 `tree_opportunity_progress`、`expected_tree_harvest_value_progress` 及当时的武器时序，
   不能把进入射程直接当作已经命中或摧毁。确认每个进入最终选择的候选都包含完整武器结果，且结果受
   预测窗内攻击次数和有限伤害容量约束，没有超过可见目标的总耐久、总移除价值或总收获价值。同一候选
   动作可以同时兑现 `material_acquisition_value` 和武器结果，复盘时不得把两类并行收益误判成取舍。
7. 武器复盘先区分自动锁定与路径碰撞：锁定目标应是攻击时刻锁定范围内的最近合法实体，首个命中事件
   则应是投送路径上最早相交且能在预测窗内到达的实体。弹射、范围效果和派生投射物按事件锚点逐级检查；
   远处目标若超过剩余飞行时间，不应贡献伤害或移除价值，随机选中该目标的概率也不应重新分配给近处
   敌人。按具体目标核对最大生命伤害、移除价值和命中后规则，避免用群体平均值掩盖目标选择偏差。常规
   `decision_sample` 只保存聚合武器结果；验证逐事件目标需要目标环境回放或先扩展诊断契约，不能从聚合值
   反推出唯一命中序列。
