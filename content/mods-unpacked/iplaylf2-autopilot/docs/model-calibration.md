...
- `decision_sample` 记录完整公共观察、此前移动输入、所选动作、预测结果、动态权重、计算预算、最多
...
`refined_action_count`、`weapon_prediction_count`、导航的 `budgeted_position_evaluation_count` 和
`estimated_work_unit_duration_usec`。三类预算内工作均为零时仍超预算，说明不可缩减的观察、碰撞成本投影
或基线评分已经超过当前同步帧余量。`decision.projectile_filter` 的纳入与延后投射物计数用于解释可达性
过滤效果。
...
| 经验启发式 | 压力曲线倍率、动态效用权重、导航吸引半径、近优带与温度、计算阶段截止比例、耗时 EMA 与保护倍率、运动估计平滑与遗忘阈值 | 不宣称物理真实性；通过当前实现产生的样本校准 |
