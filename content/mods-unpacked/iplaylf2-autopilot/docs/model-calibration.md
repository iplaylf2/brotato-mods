...
- 更长预测窗的环境暴露与碰撞诊断是“持续采用该候选动作”时的条件预测；生命、拾取与规则结果只
  结算到本次提交期，武器结果是同尺度的期望场修正，提交期外的机会由接近势能和导航终端值表达；
...
5. 材料复盘按本波剩余比例分段，比较可见材料数、实际材料增长、`material_acquisition_value` 和有符号
   ...
   形成固定击杀优先级。
6. 树木复盘同时比较 `tree_opportunity_progress`、`expected_tree_harvest_value_progress` 及当时的武器期望攻击率，
   不能把进入射程直接当作已经命中或摧毁。确认候选读取的结果场受提交期攻击容量和可见目标容量约束，
   没有超过可见目标的总耐久、总移除价值或总收获价值。同一候选
