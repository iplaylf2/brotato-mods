...
- `decision_sample` 记录公共观察的采样投影、此前移动输入、所选动作、预测结果、价值上下文、计算预算、最多
  三个参与最终选择的高分候选、最大效用选择诊断、实际使用的派生尺度、局部交互域计数与波内完成预测摘要；
...
对覆盖目标成对比较，因此密集目标下包含二次项；候选数也与动作评价成本近似线性相关。性能复盘应同时
...
补给库存使用清场时域，即时生存缓冲使用下一控制周期。设当前生命为 `H`、玩家完整动作集合、当前击退与
威胁运动在下一控制周期内形成的联合可达域中，最强接触或投射物一击经护甲折算后的伤害储备为 `R`、
...
导航只提交下一个控制周期，整波未来补给不能扩大该周期的可行生存域。这样同一份未来果实或吸血不能在
...
4. `WaveCompletionForecastModel` 的跨敌人与树木命中工作量分配、`OpportunityPricingModel` 的材料兑现
   ...
   阈值或动作禁令。道具箱的道具部分依据目标版本的共享稀有度随机数分布和当前已解锁奖池平均商店价格
   代理计算数学期望，不再把最低回收值当成整个机会。需要校准的是基础消耗品概率、其中成为箱子的条件
   ...
   局部与导航环境暴露。终止风险不应在效用账本之外形成另一套动作选择规则。
