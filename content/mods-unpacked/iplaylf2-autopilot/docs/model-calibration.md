...
不应形成固定击杀优先级。`death_rewards.material_quantity` 应等于目标版本 `get_stats_value()` 的返回值乘以
当前单位机制材料倍率；发生掉落时再与实际材料增长比较。不一致属于观察或结算契约缺陷，不是需要调节的
估计参数。材料掉落没有发生的样本只验证掉落概率，不能反推条件材料数量。
