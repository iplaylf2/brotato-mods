...
- `decision_sample` 记录公共观察的校准投影、此前移动输入、所选动作、预测结果、价值上下文、计算预算、最多
  三个完成武器预测的高分候选、最大效用选择诊断和实际使用的派生尺度；
...
`quality_mode` 则说明本轮使用完整、受限还是临界搜索基线；它由此前规划耗时决定，不代表本轮结果优劣。

`planning_duration_budget_utilization` 使用本轮实测耗时除以本轮预算。大于 `1` 表示本轮超出预算；单次
超出可能来自基线工作、工作量突变、单项耗时低估、系统调度或性能监视延迟。连续超出时，应比较
`refined_action_count`、`weapon_prediction_count`、导航的 `budgeted_position_evaluation_count`、
`estimated_work_unit_duration_usec` 与 `quality_mode`。导航预算内评价和局部细分均为零、武器预测数已降至
可行候选的比较下限，且临界质量仍然超预算时，说明最小观察、八方向近场碰撞投影或两个可比候选的武器
评分已经超过当前同步帧余量。
...
| 由状态与统一控制契约派生 | 控制/近端/局部/导航时域、有效时域、完整质量动作方向数、时间采样数、控制距离、敌人/投射物压力距离、遇敌余量、边缘与队友机动余量、漫游归一距离、机会可达距离、局部预测半径、材料份额交接区、TTC 风险与截断时域 | 不独立调参；只复核物理 tick 数、控制步数、碰撞直径倍数或解析弹道相位分辨率 |
| 稳定机制与保守代理 | 未提供标准射程时的远程压力距离、无法读取外观时的默认半径、以最大生命表示的保守耐久基准 | 保留来源和保守含义；存在更高证据等级的值时不得继续覆盖 |
| 经验启发式 | 压力曲线、生命边际价值曲线与剩余暴露折价、材料推迟兑现的成长时机价值、地图信息兑现价值、诅咒净机会价值基准、控制切换成本、近场与远场质量基线、计算阶段截止比例与质量阈值、耗时 EMA 与保护倍率、运动估计平滑与遗忘阈值 | 不宣称物理真实性；通过当前实现产生的样本校准 |
...
完整质量的动作方向数取满足相邻控制期端点弦长不超过 `r` 的最小偶数；所有质量档都保留八方向近场
安全基线。远场导航基线在完整、受限、临界质量下分别为八、六、四方向。时间采样数同时满足相邻玩家
位移不超过 `2r`、每个控制期至少一个样本、确定性曲线弹相位步长不超过 `π/2`，并且不随质量档降低。
由几何派生的完整质量方向数和时间采样数会随角色尺寸、速度、物理频率与可见弹道变化；质量基线本身属于
仍需校准的计算策略。
...
3. `PlanningComputeBudgetPolicy` 的导航/局部细分累计阶段比例、近场与远场质量基线、单项耗时 EMA、截止
   保护倍率和双偏差基线余量仍需用不同设备上的预算利用率、实际扩展数量、掉帧尖峰与行为质量共同校准。
   瞬时截止表现以
...
4. `OpportunityValueModel` 中的树木收获可行性、材料兑现价格和消耗品掉落价值，以及
   ...
   `TerminalCollisionConstraint` 的等价带只处理可能直接结束本局的风险，不得扩张为普通承伤预算。
...
6. 树木复盘同时比较 `tree_opportunity_progress`、筛选阶段的 `tree_harvest_value_in_range`、完整预测阶段的
   ...
   目标的总耐久、总移除价值或总收获价值。若筛选第一名在精确预测后收益坍塌，应先修正代理语义；至少
   两个候选的精算下限只保证结果可比较，不能掩盖代理偏差。同一候选动作可以同时兑现
...
7. 先检查 `decision.candidate_filter`：相对最低值明显更高的终止风险应在武器预测前被拒绝；所有不会直接
   ...
   的边际价值产生一次收益而不与 `consumed_consumable_recovery_supply` 相消，而
...
8. 群体接触应同时检查峰值风险、累计敌对接触、预期命中次数与原版无敌帧间隔，避免把持续贴身压缩成
   ...
   按[性能反馈的可解释范围](#性能反馈的可解释范围)检查帧耗时样本、预算利用率、截止超时量、三类预算内
