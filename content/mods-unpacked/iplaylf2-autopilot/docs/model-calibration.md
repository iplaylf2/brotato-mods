...
动作方向数上限取满足相邻控制期端点弦长不超过 `r` 的最小偶数；当轮局部动作方向数由连续搜索保真度
乘以上限后向上取整为偶数，且不少于四方向。全局导航在持续过载时可降至一对对向基线，并始终额外保留
最高价值派生方向；对向基线按低差异相位跨规划轮换，避免形成固定地图轴偏好。局部动作仍对四方向执行
完整碰撞预测。时间采样数同时满足相邻玩家位移不超过 `2r`、每个控制期至少一个样本、
...
3. `PlanningComputeBudgetPolicy` 的单项耗时 EMA、截止保护倍率和双偏差基线余量，以及
   `PlanningSearchFidelityAllocator` 的压力曲线、搜索保真度下限和导航对向基线，仍需用不同设备上的
