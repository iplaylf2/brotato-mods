...
公共价格代理形成的对局延续价值为 `C`，提交期生命状态分布中的终止概率为 `pc`，则：
...
expected_run_continuation_value_loss = pc × C
```
...
生命状态分布只有在终止分支上损失对局延续价值；局部与导航环境暴露按同一损失模型在当前时刻的
`q(0)` 计算一单位生命损失价值。
...
4. `WaveCompletionForecastModel` 的跨敌人与树木命中工作量分配、`OpportunityPricingModel` 的材料兑现
   ...
   概率，以及箱子与果实的实际兑现率。当前死亡奖励概率代理尚未包含本波
   已生成箱子数、玩家箱率修正和无尽模式消耗品衰减；比较实际兑现率时必须单独记录这些条件，不能把
   系统偏差归因于目标选择。箱子同时提供道具选择与治疗，两项收益应分别进入对应账本；已经
...
复盘消耗品或材料触发的规则时，先检查样本当时的 `observation.player_state.effect_rules`，确认事件、条件、
概率、空间交付和后果已经进入规范规则。`player_context.character_id` 只用于确认采样角色，不参与规划；
角色正确但规则缺失属于知识适配或观察契约问题，不能通过调整效用权重补偿。
