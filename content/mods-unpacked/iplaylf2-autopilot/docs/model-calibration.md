...
物理线程只执行采样准入并把已脱离场景节点的观察与计划值加入队列；采样投影、递归 JSON 转换、编码、
刷新与分片均由 `DecisionTelemetry` 的独占写入线程完成。正常关闭会等待队列排空，因此文件顺序与提交顺序
一致。序列化与 I/O 不在被采样的物理回调中执行；对帧尖峰的实际影响仍须用改动前后的同场景数据比较。
