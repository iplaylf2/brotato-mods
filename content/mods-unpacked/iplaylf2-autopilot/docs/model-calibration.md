...
截止目标访问机会只投影统一目标的采样位置，再根据玩家速度与稳定目标响应以常数时间估算首次进入锁定
区间的时间；每次场采样为 `O(N)`，并复用采样位置的敌人运动预测。它不排序目标、不展开攻击容量，也不
...
同刻零输入反事实在目标首次进入锁定区间的时间和清理前可达性上新增的价值，并受波内完成份额、存在置信度
和剩余清理窗口约束。

每把武器的最小和最大锁定距离应形成独立区间；生命目标按主路径伤害率、命中上限目标按主路径命中率
归一化各区间份额。访问值按首次进入区间的等待时间连续衰减，并在清理截止处归零；目标会自行靠近时，
接近仍应缩短等待并产生正的边际值，远离应延长等待并产生负值，但自主闭合速度会减少两者幅度。静止树木
位于最大距离外时接近应为正，位于最小距离内时远离应为正，反向移动应为负。进入区间后访问项饱和。
