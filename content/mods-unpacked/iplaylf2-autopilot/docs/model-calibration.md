...
`refined_action_count`、`weapon_prediction_count`、导航的 `extra_position_evaluation_count`、
`estimated_work_unit_duration_usec`、`budget_pressure` 与 `search_fidelity`。导航额外评价和移动角度细分均为
零、方向数已到搜索保真度下限、`weapon_scoring_mode = "screening_proxy"`，且压力饱和后仍然超预算时，应再检查
`phase_duration_usec`；这说明最小观察、四方向近场基础预测和价值导航已经超过当前同步帧余量。
...
| 经验启发式 | 生命边际价值曲线与剩余暴露折价、材料推迟兑现的成长时机价值、地图信息兑现价值、诅咒净机会价值基准、控制切换成本、预算压力指数、搜索保真度下限、即时影响压力份额、影响时间曲率、额外工作额度、耗时 EMA 与截止保护倍率、运动估计平滑与遗忘阈值 | 不宣称物理真实性；通过当前实现产生的样本校准 |
...
3. `PlanningComputeBudgetPolicy` 的单项耗时 EMA、截止保护倍率和双偏差基线余量，以及
   ...
   瞬时截止表现以
   `planning_duration_budget_utilization` 的 p95/p99 分位数和掉帧频率为验证依据；精确武器比较的增量价值应比较
...
5. 压力曲线、冲撞时间窗的可达压力、目标位置响应、地图信息兑现价值、机会可达性衰减和控制切换成本
   共同决定行为平滑度与取舍。校准必须比较候选排序、实际受伤、拾取、输出、视口有效面积和方向反转，
   不能只看最终存活。
...
6. 树木复盘同时比较 `tree_opportunity_progress`、筛选阶段的 `tree_harvest_value_in_range`、完整预测阶段的
   ...
   约束；树木代理还必须与精算使用相同的当前冷却、移动许可和最近合法目标。确认精确结果没有超过可见
   目标的总耐久、总移除价值或总收获价值。若筛选第一名在精确预测后收益坍塌，应先修正代理语义，不能
   用强制精算掩盖代理偏差。`weapon_scoring_mode` 为 `exact_comparison` 时应至少有两个精确候选；为
   `screening_proxy` 时所有候选都不应包含精确武器结果。同一候选动作可以同时兑现
   `material_acquisition_value` 和武器结果，复盘时不得把两类并行收益误判成取舍。
7. 同时检查 `terminal_collision_risk`、`expected_health_loss` 及其效用贡献：终止与非终止风险都应由统一
   效用账本与材料、输出、恢复和地图信息价值交换，不能在选择器外再次过滤。生命资源复盘按本波剩余比例
   和恢复构筑分层，对照
   ...
   `marginal_health_value`；确认普通生命价格从波初到波末连续下降、替代供给降低价格，拾取恢复按供给转化
   的边际价值产生收益，同时 `consumed_consumable_recovery_supply` 按供给影子价格结清离开地图的储备；
...
8. 群体接触应同时检查峰值风险、累计敌对接触、预期命中次数与原版无敌帧间隔，避免把持续贴身压缩成
   单次伤害；弯曲弹道应检查解析相位轨迹与扫掠交会，不能用扩大碰撞半径或加大效用权重掩盖外推错误。
   按[性能反馈的可解释范围](#性能反馈的可解释范围)检查帧耗时样本、预算利用率、截止超时量、三类额外
   工作次数和对应单项耗时 EMA；结合敌人、原始投射物及延后投射物数量解释成本变化。
