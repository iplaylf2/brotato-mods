...
5. 材料复盘按本波剩余比例分段，比较可见材料数、实际材料增长、`material_acquisition_value` 和导航中的
   ...
   确认密集材料不会退化为单个材料的封顶价值。进入吸附范围但尚未进入收集圈时，机会价值必须继续存在；
   未扫近任何材料时，路径机会不应继续提供正收益。波末样本还应确认动作、导航和 TTC 时域均不超过
   `wave_state.seconds_remaining`。动作预测窗内的拾取是持续采用该候选输入时的条件结果，不是已经执行的
   事实。另行区分导航意图中的终点总增益
