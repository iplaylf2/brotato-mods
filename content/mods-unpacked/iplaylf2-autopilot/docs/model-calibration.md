...
`has_frame_time_sample`；该值为 `false` 表示尚无有效物理回调峰值样本：本轮没有计算截止，受截止控制的
...
的比例平方得到。`search_fidelity` 记录移动搜索保真度、固定导航覆盖及离散工作额度；这些字段只解释
搜索计算分配，不代表碰撞采样精度、动作结果优劣或目标价值。导航基线除均匀方向外，至多再评价一个
...
导航额外评价和移动角度细分均为零、局部方向数已到搜索下限，且压力饱和后仍然超预算时，应再检查
...
使用最后可见时的剩余生命；没有合法生命测量时才使用稳定最大生命先验。
...
导航的武器聚群结果只选择一个预计主目标并扫描其余可见目标，每件武器为 `O(N)`，且复用同一物理帧的敌人
位置响应缓存；它不展开第二段移动候选或逐发命中图。该结果随导航位置评价次数线性增加，已经计入同一个
导航工作单元和截止策略。性能退化时应先核对位置评价数、武器数与相关敌人数，不应切换为会改变候选语义
的随机退火质量档。
...
| 经验启发式 | 生命库存的对数障碍尺度与生命补充影子价格、目标完成份额的竞争分配、材料推迟兑现的成长时机价值、材料吸收竞速时域与成长负担代理、地图再观察价值、诅咒净机会价值基准、预算压力指数、搜索保真度下限、额外工作额度、耗时 EMA 与截止保护倍率、运动估计平滑与遗忘阈值、目标响应积分的位置误差目标与最小步长 | 不宣称物理真实性或统计概率；通过当前实现产生的样本校准 |
...
动作方向数上限取满足相邻控制期端点弦长不超过 `r` 的最小偶数；当轮局部动作数由搜索保真度缩放并至少
保留四个均匀方向。全局导航保留八个均匀方向和可达机会价值上界最高的方向，后者也会进入局部候选。
...
完成份额折算的生命补充为 `S`、同期被动流失为 `D`、材料等价风险尺度为 `K`，动作预测的预期生命损失
为 `L`，则：
...
2. `MovementUtilityModel` 使用材料等价总账：一单位材料的基础价值为 `1`；收集地面材料时，
   ...
   后续波次兑现的收益。生命损失、恢复、敌人完成机会与地图信息由各自价值模型换算，普通属性变化以
   ...
   前沿容量阈值。会吸收材料的敌人按当前材料竞速和进化阈值增加存活负担；其竞速时域、逐材料成长负担和
   实际材料损失率需要单独验证。
3. `PlanningComputeBudgetPolicy` 的单项耗时 EMA、截止保护倍率和物理回调峰值余量，以及
   `PlanningSearchFidelityAllocator` 的压力曲线、搜索保真度下限、导航基线和额外工作额度，仍需用不同
...
`battlefield_effects.maximum_lifetime_hostile_population`、目标完成份额、敌人完成机会和实际新增敌人数。
...
目标切换计价。价值变化必须来自接近距离、射程与目标竞争几何，不能来自树木身份或波末专用规则。
确认候选读取的武器结果受动作预测窗攻击容量和可见目标容量约束，没有超过可见目标的总耐久、总完成价值
或总收获价值。树木价值应随箱子生成概率提高，且箱子结果仍应贡献治疗供给；箱子落地后，满血且没有
拾取事件收益时不应继续产生正向导航机会，缺血时则应与同等治疗量的果实一致。同一候选动作可以同时兑现
`material_acquisition_value` 和武器结果，复盘时不得把两类并行收益误判成取舍。
...
宝箱怪标签或固定等待奖励产生。`weapon_cluster_outcome` 是有符号结果而不是奖励字段：次要目标的净完成
价值为负时，不利结果也必须保留。若候选终点已经到达或越过波末，后续聚群结果必须为零。
