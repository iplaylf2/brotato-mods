...
- `collision` 拥有“已解析路径交会 → 按来源接触机会”和“尚无受支持轨迹的未来实现 → 未解析交会证据”
  两段协议；它消费但不拥有实体运动预测，也不解释生命损失、终止风险或动作价值；
- `weapons` 拥有“攻击模型 → 与目标无关的期望攻击容量”、“规范事件 → 动作条件攻击调度”和“武器组合与
  目标完成机制 → 目标相关的加权锁定区间”三段协议；前两者供战斗、波内完成与生命补充模型消费，后者
  供空间机会模型消费；
- `health` 拥有“接触机会与无时序聚合交会证据 → 条件生命损失、终止风险，以及有时序证据时的条件预计首次
  终止时刻”、“当前生命、即时威胁与清场前补充 →
...
- `engagement` 拥有统一可交战目标投影、敌人完成价值账本、本波共享主路径容量分配、锁定后的共享武器
  路径接触、动作条件武器结果预测与结果结算，以及规则事件完成价值。它向根目录的跨域协调者提供目标
  契约、完成份额和局部战斗结果，不拥有导航轨迹或截止目标访问价值。
...
- `bot/observation/observed_world_memory.gd` 聚合每位玩家的实体、敌人轨迹与视野覆盖记忆；
  同源节点从敌对转为非敌对时，它立即终止旧敌对轨迹，不把阵营变化误作普通失视；
...
- `bot/observation/observed_motion_estimator.gd` 保留原版为可见实体提供的当帧权威速度，并在缺少权威速度
  ...
  `bot/observation/visible_world_observer.gd` 按原版敌对目标域划分完整敌人集合：敌对的普通敌人、精英与
  Boss 形成敌人观察，转化单位形成友方观察及供记忆终止旧轨迹的同源证据；它另行输出原版持续血条提供的
  视野外存活和当前生命；对敌方投射物，它以实际命中形状中心形成位置测量，
...
- `bot/planning/battlefield_influence_model.gd` 拥有环境暴露、普通敌人与投射物的位置域碰撞证据、确定性
  ...
  该边界；
  `bot/planning/collision/contact_lookahead_projector.gd` 只把候选输入、受支持的敌人与投射物轨迹展开到
  完整可预测局部接触时域，补充近端动作窗之外的几何接触机会；它不重复计算环境、武器、拾取、规则或效用；
...
- `bot/planning/health/contact_damage_state_model.gd` 按时间顺序从接触机会推进当前生命、当前无敌剩余时间、
  受伤后变长无敌时间、闪避和命中保护的状态分布，并由该分布给出预期生命损失、累计终止概率和条件预计首次
  终止时刻；`bot/planning/health/collision_health_impact_model.gd` 仅组合该逐次状态结果与尚无逐次时刻的未解析交会证据，
  后者不伪造终止时刻；
...
- `bot/planning/movement_action_selector.gd` 只排除存在替代动作时已落入下一提交期的确定终止，随后
  最大化公共效用。完整可预测局部接触窗的碰撞与近端动作窗的其他生命变化共同形成生命库存和对局延续价值
  成本；提交期风险只拥有执行资格。选择器不拥有风险偏好、逃跑方向或敌人类别策略。
...
- `bot/planning/spatial_opportunity_value_model.gd` 是跨拾取物、规则事件、生成警告与可交战目标的空间机会
  协调者。它拥有未来玩家状态相对同刻零输入反事实的截止访问差、不可达机会剪枝，以及按价值上界形成的
  预算内搜索方向；目标运动由 `motion` 提供，目标投影与完成份额由 `engagement` 提供，武器区间及容量
  份额由 `weapons` 提供。算法语义见 [架构文档](architecture.md#暴露与导航意图)。它不猜测警告结果，
  也不解释自动选靶次序或局部攻击结果。
...
- `bot/planning/weapons/weapon_attack_schedule_model.gd` 把规范玩家规则与动作内拾取事件投影为动作条件攻击
  次数；它从 `weapon_attack_capacity_model.gd` 取得无事件基线，并拥有冷却重置后的就绪调度，不拥有拾取
  几何、目标覆盖、伤害或内容身份。
...
- `bot/planning/movement_outcome_predictor.gd` 组合动作结果；`bot/planning/movement_utility_model.gd` 将结果换算
  ...
  剩余时域裁剪的即时缓冲价值，组合生命结算窗的 `forecast_terminal_health_risk` 按其末端剩余对局延续价值计价，
  带时刻终止证据再按条件预计首次终止时刻相对战术控制期的接近程度，连续调制同一价值；
  `committed_terminal_health_risk` 只拥有执行资格。预测和评分是两个边界，
  选择器不拥有二者。
- `bot/planning/movement_action_generator.gd` 从可执行输入空间构造均匀基线，补入导航意图公开的胜出导航
  方向，并按规划器提出的细分方向构造新候选；它还从时间模型取得只随波末裁剪的近端动作窗与完整可预测
  局部接触窗，并按玩家位移和解析曲线弹相位派生近端时间采样；敌人与投射物数量不扩张任一时域；
