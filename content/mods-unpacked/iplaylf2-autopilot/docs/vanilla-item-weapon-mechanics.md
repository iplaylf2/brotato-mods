...
下表只表示相应代码路径已经接入公共模型，不单独证明每项机制的游戏内行为或数值已经校准；整体验证
状态以 [Autopilot README](../README.md) 为准。
...
| 自动目标与路径碰撞 | `Weapon.Range` 的敌人和中立单位碰撞层 | 可见敌人与树共同参与最近合法目标选择；锁定目标决定瞄准方向，路径上更早相交的实体可以先命中 |
| 穿透、弹射与暴击链 | Bandana、Ricochet、Crossbow、Shuriken | 按路径容量、逐跳伤害衰减、原版重定向概率及剩余飞行距离结算具体可达目标 |
| 命中后二次武器效果 | 爆炸武器、燃烧武器、Cactus Mace、Lightning Shiv、Sniper Gun、Vorpal Sword | 从实际命中事件按锚点、目标选择、容量和剩余飞行距离传播，不以全场目标平均值结算 |
